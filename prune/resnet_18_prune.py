...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv3x3(inplanes, planes[0][1], stride)
        self.bn1 = norm_layer(planes[0][1])
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes[1][0], planes[1][1])
        self.bn2 = norm_layer(planes[1][1])
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg=None, num_classes=1000, zero_init_residual=False,
        ...
        #stride = 1 so first layer1 doesn't need downsampling
        #every layer(1,2,3,4) contains 4 conv layers
        self.layer1 = self._make_layer(block, cfg[0:4], layers[0])
        self.layer2 = self._make_layer(block, cfg[4:8], layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, cfg[8:12], layers[2], stride=2,
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes[1][1] * block.expansion, stride),
                norm_layer(planes[1][1] * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes[0:2], stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes[1][1] * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes[2:4], groups=self.groups,
